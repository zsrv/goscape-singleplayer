# goscape-singleplayer — rev-245.2
...
This branch targets **wire-protocol revision 245.2** and is pinned to the
`rev-245.2` branches of both upstream projects. For the other revisions and the
...
- A packed revision 245.2 game cache (see below). This repository ships no game
  assets.
...
the `rev-245.2` branch, `make pack` produces one; point `--cache-dir` at its
