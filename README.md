...
This branch targets **wire-protocol revision 225** and is pinned to the
`rev-225` branches of both upstream projects. For the other revisions and the
...
- A packed rev-225 game cache, **when building from source** (see below).
  Release binaries carry one already — see [Downloads](#downloads).
...
[goscape](https://github.com/zsrv/goscape) checkout on the `rev-225` branch, `make pack` produces one; point `--cache-dir` at its
output (default `./data/pack`).
...
|---|---|---|
| `--cache-dir` | `./data/pack` | packed game cache (split layout: `client/` + `server/`) |
