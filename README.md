...
This branch's `replace` targets are the `../goscape-rev244` and
`../goscape-client-rev244` sibling worktrees — no branch-switching needed in
either sibling, they're already parked on their rev-244 tips.

Build (CGO required — GLFW/OpenGL):

    CGO_ENABLED=1 go build -o goscape-singleplayer ./cmd/goscape-singleplayer

You need a packed game cache. In the goscape-rev244 repo, `make pack`
produces one; point `--cache-dir` at its output (default `./data/pack`).

    ./goscape-singleplayer --cache-dir ../goscape-rev244/data/pack
